...
/*   Updated: 2025/08/24 14:04:30 by mansargs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	create_philos_semaphores(t_philo *philo, int num)
{
	...
	if (!name)
		return (printf("033[0;31Memory allocation failed\033[0m"), false);
	philo->last_meal_sem = sem_open(name, O_CREAT | O_EXCL, 0644, 1);
	free(name);
	name = individual_sem_name("/meals_eaten_sem", num);
	if (!name)
		return (printf("033[0;31Memory allocation failed\033[0m"), false);
	philo->meals_eaten_sem = sem_open("/meals_eaten_sem", O_CREAT | O_EXCL, 0644, 1);
	free(name);
	if (philo->last_meal_sem == SEM_FAILED || philo->meals_eaten_sem == SEM_FAILED)
		return (printf("033[0;31Semaphore opening failed\033[0m"), false);
}
